...
void main() {
  ...
  test('libDirOnPath puts the bundle lib dir on LD_LIBRARY_PATH', () async {
    // Vendored libraries carry no RUNPATH, so a library one of them needs is
    // only found through the loader path.
    await FlatpakPackager(runProcess: fakeRun).build(
      bundleDir: fakeBundle(),
      outDir: Directory(p.join(tmp.path, 'dist')),
      meta: const FlatpakMetadata(
        ...
        command: 'homescreen',
        libDirOnPath: true,
        env: {'LD_LIBRARY_PATH': '/app/com.example.App/lib'},
      ),
    );
    const keep = r'${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}';
    final lines = capturedLauncher!.trim().split('\n');
    expect(lines[1], 'export LD_LIBRARY_PATH="/app/com.example.App/lib$keep"');
    // The identical manifest entry is not written a second time.
    expect(lines.where((l) => l.contains('LD_LIBRARY_PATH=')), hasLength(1));
  });

  test('args with shell metacharacters are quoted', () async {
    await FlatpakPackager(runProcess: fakeRun).build(
      bundleDir: fakeBundle(),
      outDir: Directory(p.join(tmp.path, 'dist')),
      meta: const FlatpakMetadata(
        appId: 'com.example.App',
        command: 'homescreen',
        args: ["--title=Bob's", '--x;reboot', '--glob=*.json', r'--home=$HOME'],
      ),
    );
    expect(
      capturedLauncher!.trim().split('\n').last,
      'exec /app/com.example.App/homescreen -b /app/com.example.App '
      '"--title=Bob\'s" "--x;reboot" "--glob=*.json" "--home=\$HOME" "\$@"',
    );
  });
